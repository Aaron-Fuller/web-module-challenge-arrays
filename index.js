...
function getFlavorByIndex(originalFlavors, indexOfFlavor){
    console.log(originalFlavors[indexOfFlavor]);
}
...
function removeFlavorByName(originalFlavors, flavorName){
    let newIndex = originalFlavors.indexOf(flavorName);
    originalFlavors.splice(newIndex, 1);
    console.log(originalFlavors);
}
...
function filterByWord(originalFlavors, arg){
    let seasonal = [];
    
    for (i = 0; i < originalFlavors.length; i++) {
        if (originalFlavors[i].includes(arg, )) {
            seasonal.push(originalFlavors[i]);
        }
    }
...
function getAverageWordLength(dataToProcess){
    ...
    for (i = 0; i < dataToProcess.length; i++) {
        newSet.push((dataToProcess[i].split(' ').length))
    }
    console.log(newSet.reduce(function(acc, val) { return acc + val; }, 0)/newSet.length)
}
